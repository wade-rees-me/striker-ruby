...
# Represents a shoe of playing cards used in a game of Blackjack.
class Shoe
  attr_reader :number_of_cards

  SUITS = %w[spades diamonds clubs hearts].freeze
  RANKS = {
    'two' => 2, 'three' => 3, 'four' => 4, 'five' => 5,
    'six' => 6, 'seven' => 7, 'eight' => 8, 'nine' => 9,
    'ten' => 10, 'jack' => 10, 'queen' => 10, 'king' => 10, 'ace' => 11
  }.freeze
  KEYS = %w[2 3 4 5 6 7 8 9 X X X X A].freeze

  def initialize(number_of_decks, penetration)
    initialize_cards(number_of_decks)
  ...
  def initialize_cards(number_of_decks)
    @cards = []
    number_of_decks.times do
      SUITS.each do |suit|
        RANKS.each_with_index do |(rank, value), offset|
          @cards << Card.new(suit, rank, KEYS[offset], value)
        end
      end
    end
  end
